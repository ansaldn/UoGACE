...
gpio.setup(trigger, gpio.OUT) # Ultrasonic trigger
gpio.setup(echo, gpio.IN)# ultrasonic echo

bottomPos = gpio.PWM(servo1, 200) 
topPos = gpio.PWM(servo2, 200)

#Servo positions (based on testing with my set up; may change if screws are tightened)
left = 25/5
right = 150/5
centre = 50/5
up = 50/5
...
# to be used if the rover hits a dead end where left, right and ahead is blocked
...
def ultrasonicDistance(): #calculates distance from objects ahead and determins if the way is clear
    while True:
            ...
            while gpio.input(echo) == 0: #count elapsed time between the sent and received signal
